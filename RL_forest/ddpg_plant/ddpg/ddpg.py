...
from RL_forest.ddpg_plant.common.util import reduce_std
...
class DDPG(object):
    def __init__(self, actor, critic, memory, observation_shape, action_shape, action_noise=None,
                 ...
                 actor_lr=1e-4, critic_lr=1e-3, clip_norm=None, reward_scale=1., baseline=True):
        # train input
        ...
        # apply baseline to the Policy gradient
        ...
        if self.baseline:
            ...
            self.target_values = [self.target_Q, self.target_V]
        else:
            self.critic_tf = critic(self.obs0, self.actions, baseline=False)
            self.critic_with_actor_tf = critic(self.obs0, self.actor_tf, reuse=True, baseline=False)
            Q_obs1 = target_critic(self.obs1, target_actor(self.obs1))
            self.target_Q = self.rewards + (1. - self.terminals1) * gamma * Q_obs1
            self.target_values = [self.target_Q]
    ...
    def setup_target_network_updates(self):
        ...
        # baseline variables will be included if set to true.
    ...
    def setup_critic_optimizer(self):
        ...
        if self.baseline:
            self.critic_loss += tf.reduce_mean(tf.square(self.value_tf - self.value_target))
    ...
    def train(self):
        ...
        # TODO Run experiments to verify the whole code
        target_values = self.sess.run(self.target_values, feed_dict={
        ...
        train_feed_dict = {
            ...
            self.critic_target: target_values[0],
        }

        if self.baseline:
            train_feed_dict[self.value_target] = target_values[1]

        # Get all gradients and perform a synced update.
        ops = [self.actor_train_op, self.actor_loss, self.critic_train_op, self.critic_loss]
        _, actor_loss, _, critic_loss = self.sess.run(ops, feed_dict=train_feed_dict)
